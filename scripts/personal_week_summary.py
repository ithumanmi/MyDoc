...
def compute_score(stats: WeekStats, cfg: dict) -> dict[str, float]:
    ...
    wo_target = float(t.get("workouts_week") or 3)
    wo_ratio = min(1.0, stats.workouts / wo_target) if wo_target else 0
    workout_part = wo_ratio * w.get("workouts", 0)
...
def render(year: int, week: int, days: list[date], stats: WeekStats, parts: dict, cfg: dict) -> str:
    ...
    lines = [
        ...
        f"| Workouts | {parts.get('workouts', 0)} | {cfg['weights'].get('workouts', 0)} |",
        ...
        f"- Workouts (CSV training): {stats.workouts} / target {cfg['targets'].get('workouts_week', 3)}",
        f"- Nutrition files: {stats.nutrition_days}/7 · Metrics rows: {stats.metrics_days}/7",
        f"- Habits: {fmt(stats.habit_ratio, 2)} of marked cells ({stats.habit_cells} cells)",
        f"- Avg energy (from daily): {fmt(stats.avg_energy)}",
        f"- Mind scores (Anxiety/Recovery/Social): review daily files manually — not in /100 yet",
        "",
