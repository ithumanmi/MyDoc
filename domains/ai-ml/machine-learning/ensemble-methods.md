...
Thay vì dựa vào một model đơn, ta kết hợp nhiều model lại để giảm phương sai, giảm bias hoặc tăng tính ổn định. Ensemble là bí kíp vô địch Kaggle, từ Random Forest đến Gradient Boosting.
...
> ⚙️ Workflow: Chuẩn hóa pipeline → train RF với `n_estimators=500` + monitor OOB → dùng SHAP/Permutation để chọn feature rồi fine-tune model boosting.
...
4. **Pipeline:** Kết hợp với feature engineering, hyperparameter tuning.
5. **Khi nào dùng?** Khi single model đã tối ưu mà vẫn muốn tăng thêm vài % accuracy hoặc cần model ổn định.
...
> 💡 Tip: Với dữ liệu lớn, bắt đầu bằng LightGBM/XGBoost với early stopping để có baseline mạnh. Khi cần thêm vài % cuối, thử stacking hoặc blending.
