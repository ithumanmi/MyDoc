...
graph TD
    ...
    B --> B1(Blockchain Structure)
    B --> B2(Consensus: PoW/PoS)
    B --> B3(Cryptography: Hash/Keys)

    B --> C{Choose Your Path}

    C --> D[🛠️ Developer]
    D --> D1(Smart Contracts: Solidity/Rust)
    D --> D2(Frontend: Web3.js/Ethers.js)
    D --> D3(DApps Architecture)

    C --> E[💰 DeFi & Investing]
    E --> E1(Tokenomics)
    E --> E2(DeFi: AMM/Lending)
    E --> E3(On-chain Analysis)

    C --> F[🛡️ Security Auditor]
    F --> F1(Common Vulnerabilities)
    F --> F2(Auditing Tools)
    F --> F3(Hack Analysis)
```
