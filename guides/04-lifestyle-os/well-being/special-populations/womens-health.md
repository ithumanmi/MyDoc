...
```mermaid
pie showData
    title 28-Day Cycle Emphasis
    "Menstrual (Rest/Recovery)" : 5
    "Follicular (Build/Focus)" : 7
    "Ovulatory (Social/Performance)" : 4
    "Luteal (Stabilize/Calm)" : 12
```

```mermaid
flowchart LR
    Menstrual(Rest & Sleep Hygiene) --> Follicular(Deep Work + Heavy Strength)
    Follicular --> Ovulatory(Social + High-stakes Meetings)
    Ovulatory --> Luteal(Glucose Stability + Breathwork)
    Luteal --> Menstrual
    Luteal --> Recovery(Taper caffeine, add magnesium)
    Recovery --> Menstrual
```
