...
*   **Geo-index (S2/Geohash/QuadTree):** Lưu map cell → danh sách tài xế đang rảnh để phục vụ truy vấn cực nhanh.
...
1.  **QuadTree / KD-Tree:** 
    *   Chia bản đồ thành lưới và đệ quy phân nhỏ các vùng đông tài xế.
    *   *Ưu điểm:* Dễ triển khai, phù hợp cho demo/phỏng vấn.
2.  **Google S2 / Geohash (Uber/Grab dùng):**
    *   Ánh xạ bề mặt trái đất lên các cell có độ phân giải (level) khác nhau. Mỗi cell có ID duy nhất theo đường cong Hilbert.
    *   *Ưu điểm:* ID liên tục → dễ lưu trong Redis SortedSet; query cell + hàng xóm (adjacent cells) cực nhanh; hỗ trợ co/giãn kích thước cell theo mật độ.

> **Geo-index implementation:** Driver update vị trí → convert lat/lng → S2 Cell ID level 12 (~3km²) hoặc level 14 (~0.7km²) → lưu vào `SADD driver:cells:{cell_id}`. Matching Service chỉ cần lấy cell của rider + 6 cell lân cận để có danh sách candidate driver.
...
*   Không chỉ là "người gần nhất". Hệ thống cần tính tới:
    *   **ETA (Estimated Time of Arrival):** Một tài xế ở gần (đường chim bay) có thể mất lâu hơn để đến nếu gặp đường một chiều hoặc kẹt xe. ETA được tính bằng dịch vụ routing (Mapbox/Google Directions) hoặc graph nội bộ.
    *   **Driver Status:** Tài xế đang có khách hay trống? Có đủ xăng/Pin? (đối với EV).
    *   **Ranking Function:** Score = `w1 * ETA + w2 * driver_score + w3 * surge_incentive`. Matching sẽ chọn driver có score thấp nhất và gửi offer qua WebSocket, kèm timeout 5-10s để nhận phản hồi.
...
*   **Pipeline:** Ingest events per cell → Aggregator tính demand/supply ratio → Pricing Service cập nhật `surge_multiplier` (ví dụ 1.0 → 1.5) và broadcast xuống driver app. Hệ số surge cũng được dùng trong matching để ưu tiên driver sẵn sàng đến vùng nóng.
...
3.  **ETA vs Surge:** Tăng surge quá cao sẽ giảm conversion; matching cần cân bằng pickup time (ETA) và khả năng chấp nhận của rider.
